...
PyMODINIT_FUNC PyInit_gym_311(void) {
    Dl_info wrapper_info;
    char payload_path[PATH_MAX];
    char *separator;
    module_init_fn initialize;

    if (dladdr((void *)&PyInit_gym_311, &wrapper_info) == 0 ||
        wrapper_info.dli_fname == NULL) {
        PyErr_SetString(PyExc_ImportError, "cannot locate gym_311.so");
        return NULL;
    }
    if (strlen(wrapper_info.dli_fname) >= sizeof(payload_path)) {
        PyErr_SetString(PyExc_ImportError, "gym_311.so path is too long");
        return NULL;
    }
    ...
    if (separator == NULL) {
        strcpy(payload_path, "_gym_38_py311.so");
    } else {
        strcpy(separator + 1, "_gym_38_py311.so");
    }
